/**
 * @file SQLiteCustomerRepo.h
 * @brief SQLite 数据仓库实现类声明。
 *
 * 研究定位：本文件属于数据访问实现层，声明 ICustomerRepository 的 SQLite 版本。
 * 它是业务对象与本地 crm.db 数据库之间的主要转换边界。
 *
 * 主要职责：声明用户、客户、跟进记录、公海池和客户交接的数据库操作，以及建表、
 * 初始数据写入和从 QSqlQuery 转换模型对象的辅助方法。
 */
